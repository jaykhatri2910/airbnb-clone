...
/* ── Thin SVG icon wrapper ── */
const Icon = ({ children }) => (
  <span
    className="ico"
    style={{ width: 24, height: 24, display: "inline-flex", flexShrink: 0 }}
  >
    <svg
...
const CloseBtn = ({ onClick }) => (
  <button
    ...
    onMouseEnter={(e) => (e.currentTarget.style.background = "var(--grey200)")}
...
/* ── One amenity row ── */
const Row = ({ label, unavailable, icon }) => (
  <div
    className={`modal-amenity-item${unavailable ? " amenity--unavailable" : ""}`}
  >
    <Icon>{icon}</Icon>
...
const Group = ({ title, items }) => (
  <div className="amenity-group">
    <h3>{title}</h3>
    {items.map(({ label, unavailable, icon }) => (
      <Row key={label} label={label} unavailable={unavailable} icon={icon} />
    ))}
  </div>
);
...
const CATEGORIES = [
  {
    title: "Bathroom",
    items: [
      { label: "Hairdryer", icon: icons.hairdryer },
      { label: "Cleaning products", icon: icons.cleaning },
      { label: "Shampoo", icon: icons.shampoo },
      { label: "Hot water", icon: icons.hotwater },
      { label: "Shower gel", icon: icons.showergel },
    ],
  },
  {
    title: "Bedroom and laundry",
    items: [
      { label: "Washing machine", icon: icons.washer },
      { label: "Hangers", icon: icons.hanger },
      { label: "Bed linen", icon: icons.bedlinen },
      { label: "Room-darkening blinds", icon: icons.blinds },
      { label: "Iron", icon: icons.iron },
      { label: "Clothes storage", icon: icons.storage },
      { label: "Cot", icon: icons.cot },
    ],
  },
  {
    title: "Entertainment",
    items: [{ label: "TV", icon: icons.tv }],
  },
  {
    title: "Family",
    items: [{ label: "Cot", icon: icons.cot }],
  },
  {
    title: "Heating and cooling",
    items: [
      { label: "Air conditioning", icon: icons.ac },
      { label: "Ceiling fan", icon: icons.fan },
    ],
  },
  {
    title: "Home safety",
    items: [
      { label: "Exterior security cameras on property", icon: icons.camera },
      {
        label: "Carbon monoxide alarm",
        icon: icons.comonoxide,
        unavailable: true,
      },
      { label: "Smoke alarm", icon: icons.smokealarm, unavailable: true },
    ],
  },
  {
    ...
    items: [
      ...
      { label: "Dedicated workspace", icon: icons.workspace },
    ],
  },
  {
    ...
    items: [
      ...
      { label: "Fridge", icon: icons.fridge },
      { label: "Freezer", icon: icons.freezer },
      { label: "Microwave", icon: icons.microwave },
      { label: "Cooking basics", icon: icons.cooking },
      { label: "Crockery and cutlery", icon: icons.crockery },
      { label: "Kettle", icon: icons.kettle },
      { label: "Coffee", icon: icons.coffee },
      { label: "Wine glasses", icon: icons.wineglass },
      { label: "Toaster", icon: icons.toaster },
      { label: "Blender", icon: icons.blender },
      { label: "Cooker", icon: icons.cooker },
    ],
  },
  {
    title: "Location features",
    items: [{ label: "Private entrance", icon: icons.entrance }],
  },
  {
    title: "Outdoor",
    items: [
      { label: "Patio or balcony", icon: icons.patio },
      { label: "Outdoor dining area", icon: icons.dining },
    ],
  },
  {
    title: "Parking and facilities",
    items: [
      { label: "Free parking on premises", icon: icons.parking },
      { label: "Pool", icon: icons.pool },
      { label: "Hot tub", icon: icons.hottub },
  ...
  },
  {
    title: "Services",
    items: [
      { label: "Pets allowed", icon: icons.pets },
      { label: "Cleaning available during stay", icon: icons.cleaning2 },
      { label: "Long-term stays allowed", icon: icons.longterm },
      { label: "Self check-in", icon: icons.selfcheckin },
    ],
  },
];
    ...
    >
      <div className="modal-dialog">
        ...
        <div className="modal-body">
          ...
          {CATEGORIES.map((cat) => (
            <Group key={cat.title} title={cat.title} items={cat.items} />
          ))}
        </div>
      </div>
    </div>
